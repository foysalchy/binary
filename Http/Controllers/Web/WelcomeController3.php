...
// 		$upcomingProducts = Product::where('status', 1)->where('stock_status', 'upcoming')->orderBy('id', 'desc')->take(8)->get();
        ...
        /*$offer_products = DB::table('products')->select(
            'products.id','name','subtitle','slug','buying_price','discount_price','discount','regular_price',
            'special_price','offer_price','price_highlight','call_for_price','product_image_small',
            'image','image_alt','image_des','status','sku','product_stock_statuses.stock_status'
            )
       ...
       // $s_f_box = BGColor::where('section', '=', 's_f_box')->where('status', 1)->first();
		...
		return view('welcome', compact(
            ...
            //'subCategories',
           // 'sliderTexts', 
           // 'first4shopTypes',   'second4shopTypes',  'third4shopTypes', 
            //'latestProducts', 
            //'offer_products', 
            //'topProducts', 
            'SiteSetting',
            //'f_f_box', 's_f_box', 't_f_box', 'f_box_card', 'f_box_card_font_color', 
            //'FeatureBoxFAds', 'FeatureBoxSAds', 'FeatureBoxTAds', 
            //'animated_text_bg', 'animated_text_font_color', 'animated_text_fontsize', 
            //'sidebarads', 'upcomingProducts'
        ));
        /* 'posts',*/
	}
