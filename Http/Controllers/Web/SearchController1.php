    ...
    public function MakeUrl($slug)
    {
        ...
        if ($category) {
            if($category->status == 1){
                ...
                return view('frontend.product.layoutcategory', compact('products', 'cat_own','cat_child'));
            }else{
        ...
        if ($subcategory) {
            if($subcategory->status == 1){
                ...
                return view('frontend.product.layoutcategory', compact('products', 'cat_parent','cat_own','cat_child'));
            }else{
                return view('frontend.fourzeofour');
            }            
        }
        ...
        if ($prosubcategory) {
            ...
            $cat_child = Proprocategory::where('pro_sub_category_id', $prosubcategory->id)
                ...
                ->select('name','slug')
                ->get();
            $products = Product::where('category_id', $prosubcategory->category_id)->where('status', 1)->where('sub_category_id', $prosubcategory->subcategory_id)->where('pro_sub_category_id', $prosubcategory->id)->orderBy('created_at', 'desc')->paginate(12);
            return view('frontend.product.layoutcategory', compact('products','cat_parent_1','cat_parent','cat_own','cat_child'));
        }
        ...
        if ($proprocategory) {
            ...
            return view('frontend.product.layoutcategory', compact('products','cat_parent_2','cat_parent_1','cat_parent','cat_own'));
        }
        ...
        if ($product) {
            if($product->status == 1){
                ...
                return view('frontend.product.layoutproduct', compact(
                    ...
                    'ProductHighlights', 'ProductTerms', 'FreeItemForClients'
                ));
            }else{
                return view('frontend.fourzeofour');
            }
